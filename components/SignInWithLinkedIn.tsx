...
interface SignInWithLinkedInProps {
  className?: string;
...
export default function SignInWithLinkedIn(
  { className, isHistorical = false, contentToPost, chatId }: SignInWithLinkedInProps
) {
  const { user, isAuthenticated, hasLinkedInAccess, isLiveChat, isLoading } = useAuth();

  const handleSignIn = useCallback(() => {
    // Store content and flag in sessionStorage before signing in
    if (contentToPost && chatId) {
      sessionStorage.setItem(`pending_post_content_${chatId}`, contentToPost);
      sessionStorage.setItem(`pending_post_flag_${chatId}`, 'true');
      sessionStorage.setItem(`pending_post_timestamp_${chatId}`, Date.now().toString());
    }

    signIn('linkedin');
  }, [contentToPost, chatId]);
